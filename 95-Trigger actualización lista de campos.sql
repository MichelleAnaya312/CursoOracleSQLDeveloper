...
--5 
/*Creamos un desencadenador a nivel de fila que se dispare cada vez que se actualiza 
el campo "precio"; el trigger debe ingresar en la tabla "control", el nombre del 
usuario, la fecha y la hora en la cual se realizó un "update" sobre "precio" de 
"libros":*/
...
--7 Aumentamos en un 10% el precio de todos los libros de editorial "Nuevo siglo':
update libros set precio=precio+precio*0.1 where editorial='Nuevo siglo';

--8 Veamos cuántas veces se disparó el trigger consultando la tabla "control":
select * from control;
...
--9 Modificamos otro campo, diferente de "precio":
update libros set autor='Lewis Carroll' where autor='Carroll';

--10 Veamos si el trigger se disparó consultando la tabla "control":
select * from control;
/*El trigger no se disparó (no hay nuevas filas en "control"), pues está definido 
solamente sobre el campo "precio".*/